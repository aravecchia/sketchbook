...
// bits for each LED will be set to 1 to turn on LED
void setup()
{
...
void loop() {
  ...
  {
    if (led < ledLevel ) {
      bitWrite(ledBits, led, HIGH);
      // turn on LED if less than the level
    }
  else {
    bitWrite(ledBits, led, LOW);
      // turn off LED if higher than the level
    }
